...
class QecZCheck(EnvExperiment):
    ...
    def run(self):
        ...
        for err, (exp_syn, exp_flag) in EXPECT:
            if err is not None:
                self.qec.inject_x(err) # laser flip on one data qubit
            s0, s1  = self.qec.extract_syndrome(measure_duration) # routing + mid-circuit measure+reset
            flagged = self.qec.correct(s0, s1) # decode + corrective X
            logical = self.qec.logical_readout(measure_duration)  # measured majority vote
            self.shuttling.route_home() # reset of positions -> INITIAL
    ...
    def analyze(self):
        ...
        fig.tight_layout()
        fig.savefig("qec_x_check.pdf", bbox_inches="tight"); plt.close(fig)